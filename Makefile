...
MARKDOWN := pandoc --template $(TEMPLATE) -c $(CSS) --from markdown+inline_notes --to html5 --standalone 
...
EXCLUDE_TYPES = $(PROCESS_TYPES) $(CONTROL_TYPES) $(DEPLOY_DIRECTORY)% $(SRC_CTL) ./%.DS_Store %.py %.txt %.json
